...
import { items, shuffleItems, type Item } from '../data/rwa-items';
import {
  type Response,
  calculateAllScores,
  serializeResponses,
...
const STORAGE_KEY = 'mesearch-rwa-responses';
...
const likertOptions = [
  { value: -4, label: 'Very Strongly Disagree' },
  ...
  { value: 4, label: 'Very Strongly Agree' },
];

export default function RWAAssessment() {
          ...
          <div className="card-premium rounded-lg p-10">
            <div className="text-center mb-8">
              ...
              <p className="text-[var(--color-text-muted)] text-sm">
                Very Short Scale (RWA-VSA)
              </p>
            </div>

            <div className="space-y-6 text-[var(--color-text-secondary)] text-sm leading-relaxed">
              <p>
                This assessment measures attitudes across three dimensions:
                <strong className="text-[var(--color-text-primary)]"> Authoritarian Submission</strong>,
                <strong className="text-[var(--color-text-primary)]"> Authoritarian Aggression</strong>, and
                <strong className="text-[var(--color-text-primary)]"> Conventionalism</strong>.
              </p>
              ...
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 text-blue-200">
                ...
                <p className="text-sm text-blue-200/80">
                  This scale measures <strong>attitudes about authority and tradition</strong>, not
                  character or moral worth. These attitudes exist on a spectrum and are influenced by
                  culture, upbringing, and life experiences. The term &quot;right-wing&quot; in this context
                  is a historical research label, not a political party affiliation.
                </p>
              </div>
              ...
              <div className="bg-[var(--color-bg-secondary)] rounded-lg p-6 space-y-4">
                ...
                <ul className="list-disc list-inside space-y-2 text-[var(--color-text-muted)]">
                  <li>This test contains 6 questions</li>
                  <li>It takes approximately 2 minutes to complete</li>
              ...
              <p className="text-[var(--color-text-muted)] text-xs">
                For each statement, indicate how strongly you agree or disagree on a 9-point scale
        ...
        <main className="mx-auto max-w-2xl px-6 py-16">
          <div className="card-premium rounded-lg p-10 text-center">
            ...
            <p className="text-[var(--color-text-secondary)] mb-8">
              Thank you for completing the Right-Wing Authoritarianism assessment. Your results are ready.
            </p>
            <button
              onClick={() => navigate('/test/rwa/results')}
              className="btn-gold px-10 py-4 rounded text-sm tracking-widest uppercase"
              data-testid="rwa-view-results"
            >
              View Results
            </button>
          </div>
      ...
      <header className="border-b border-[var(--color-border-subtle)] bg-[var(--color-bg-primary)]/95 backdrop-blur-md transition-colors duration-300">
        <div className="mx-auto max-w-3xl px-6 py-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[var(--color-text-muted)] text-sm" data-testid="rwa-progress">
          ...
          <div className="space-y-2">
            {likertOptions.map((option) => (
              <button
                ...
                data-testid={`rwa-option-${option.value}`}
              >
                <div className="flex items-center gap-4 p-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] hover:border-[var(--color-champagne)]/50 hover:bg-[var(--color-bg-secondary)]/80 transition-all duration-200">
                  <div className="w-8 h-8 rounded-full border-2 border-[var(--color-border)] group-hover:border-[var(--color-champagne)] flex items-center justify-center transition-colors duration-200">
                    <span className="text-[var(--color-text-muted)] group-hover:text-[var(--color-champagne)] text-xs font-medium transition-colors duration-200">
                      {option.value > 0 ? `+${option.value}` : option.value}
                    </span>
                  </div>
                  <span className="text-[var(--color-text-secondary)] group-hover:text-[var(--color-text-primary)] text-sm transition-colors duration-200">
                    {option.label}
                  </span>
                </div>
              </button>
