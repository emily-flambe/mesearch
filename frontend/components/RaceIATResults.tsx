...
interface RaceIATResultsDisplayProps {
  results: IATResults;
  onRetake: () => void;
}

/**
 * Get Race IAT-specific interpretation text based on D-score.
 *
 * For Race IAT:
 * - Positive D = slower when White faces + Good share a key = implicit preference for Black faces
 * - Negative D = slower when Black faces + Good share a key = implicit preference for White faces
 *
 * Note: The "compatible" pairing in our implementation has Black faces + Good together initially.
 * A positive D-score means the participant was faster on that pairing.
 */
function getRaceIATInterpretation(dScore: number): string {
  if (dScore < -0.65) {
    return 'Your result suggests a strong automatic preference for White faces compared to Black faces. This means you were noticeably faster when White faces and Good shared a response key. This is a common pattern among test-takers.';
  }
  if (dScore < -0.35) {
    return 'Your result suggests a moderate automatic preference for White faces compared to Black faces. This means you were faster when White faces and Good shared a response key. This is a common pattern among test-takers.';
  }
  if (dScore < -0.15) {
    return 'Your result suggests a slight automatic preference for White faces compared to Black faces. This means you were somewhat faster when White faces and Good shared a response key.';
  }
  if (dScore <= 0.15) {
    return 'Your result suggests little to no automatic preference between Black faces and White faces. Your response times were similar regardless of which concepts shared a response key.';
  }
  if (dScore <= 0.35) {
    return 'Your result suggests a slight automatic preference for Black faces compared to White faces. This means you were somewhat faster when Black faces and Good shared a response key.';
  }
  if (dScore <= 0.65) {
    return 'Your result suggests a moderate automatic preference for Black faces compared to White faces. This means you were faster when Black faces and Good shared a response key.';
  }
  return 'Your result suggests a strong automatic preference for Black faces compared to White faces. This means you were noticeably faster when Black faces and Good shared a response key.';
}

export default function RaceIATResultsDisplay({ results, onRetake }: RaceIATResultsDisplayProps) {
  ...
  // Get race-specific interpretation
  const interpretationText = getRaceIATInterpretation(results.dScore);

  return (
    <div className="flex-1 flex items-center justify-center p-6" data-testid="race-iat-results">
      <div className="card-premium rounded-lg p-8 md:p-12 max-w-2xl w-full">
        <div className="text-center mb-8">
          ...
          <div className="mb-6">
            ...
            >
              {badge.text} Automatic Preference
            </span>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex justify-between text-xs text-[var(--color-text-muted)] mt-1">
              <span>White + Good</span>
              <span>Black + Good</span>
            </div>
          </div>
        </div>
        ...
        {/* Critical Disclaimer for Race IAT */}
        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-6">
          ...
          <ul className="text-[var(--color-text-muted)] text-xs space-y-2 leading-relaxed">
            <li>
              <strong>NOT a measure of racism.</strong> An implicit preference does not mean you are racist or consciously prejudiced. These are automatic associations that can exist even in people who genuinely hold egalitarian values.
            </li>
            <li>
              <strong>NOT a character judgment.</strong> Your implicit associations do not define who you are as a person. They reflect cultural exposure, not personal endorsement.
            </li>
            <li>
              <strong>NOT predictive of individual behavior.</strong> A single IAT score does not reliably predict how you will act in any specific situation.
            </li>
          </ul>
        </div>
        ...
        <div className="bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-lg p-4 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[var(--color-text-muted)] text-xs">
                This version uses placeholder text labels instead of face images.
                The standard Race IAT uses photographs of Black and White faces.
        ...
        <div className="bg-[var(--color-bg-tertiary)] rounded-lg p-4 mb-8">
          ...
          <div className="space-y-2 text-xs">
            ...
            >
              ...
              Take the Official Race IAT - Project Implicit
            </a>
            <a
            ...
            <a
              href="https://www.kirwaninstitute.osu.edu/research/understanding-implicit-bias"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 text-[var(--color-text-muted)] hover:text-[var(--color-champagne)] transition-colors"
            >
