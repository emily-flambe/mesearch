...
import RaceIATResultsDisplay from './RaceIATResults';
import { useAuth } from '../contexts/AuthContext';

type Phase = 'intro' | 'instructions' | 'trial' | 'block-break' | 'results';

const STORAGE_KEY = 'mesearch-race-iat-progress';
const INTER_TRIAL_INTERVAL = 250; // ms between trials

/**
 * Race IAT Assessment Component
 *
 * This component implements the Race IAT, which measures implicit associations
 * between Black/White faces and Good/Bad words. It follows the standard IAT
 * methodology with additional ethical disclaimers appropriate for measuring
 * racial bias.
 *
 * IMPORTANT ETHICAL NOTES:
 * - This is presented as an educational self-reflection tool, NOT diagnostic
 ...
 * - Results should NOT be used for hiring, selection, or consequential decisions
 */
export default function RaceIATAssessment() {
  const { user } = useAuth();
  const [baseConfig] = useState<IATConfig>(raceIAT);
  ...
  // Counterbalancing per IAT methodology (2x2 design = 4 conditions):
  // 1. pairingOrderSwapped: Which combined pairing comes first (compatible vs incompatible)
  // 2. targetSideSwapped: Which target starts on the left (Black faces vs White faces)
        ...
        {phase === 'intro' && (
          <div className="flex-1 flex items-center justify-center p-6">
            <div className="card-premium rounded-lg p-12 max-w-2xl w-full text-center">
              ...
              {/* Critical warning box for sensitive test */}
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-6 text-left">
                <p className="text-red-400 text-sm font-medium mb-2">
                  Sensitive Assessment
                </p>
                <p className="text-[var(--color-text-muted)] text-xs">
                  This test measures implicit racial associations. Your results may be surprising or uncomfortable.
                  Please read the full disclaimer below before proceeding.
                </p>
              </div>

              {/* Disclaimer box */}
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 mb-8 text-left max-h-64 overflow-y-auto">
              ...
              <div className="space-y-4">
                <p className="text-[var(--color-text-secondary)] text-sm">
                  You will categorize words and labels using the E and I keys as quickly as possible.
                </p>
                <button
                  onClick={startIAT}
                  className="btn-gold px-10 py-4 rounded text-sm tracking-widest uppercase"
                  data-testid="race-iat-start"
                >
                  Begin Assessment
                </button>
              </div>
          ...
          <div className="flex-1 flex items-center justify-center p-6">
            <div className="card-premium rounded-lg p-12 max-w-xl w-full text-center">
              ...
              <button
                onClick={beginTrials}
                className="btn-gold px-10 py-4 rounded text-sm tracking-widest uppercase"
                data-testid="race-iat-begin-block"
              >
                Start Block
              </button>
