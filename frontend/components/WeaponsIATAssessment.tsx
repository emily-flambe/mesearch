...
import WeaponsIATResults from './WeaponsIATResults';
import { useAuth } from '../contexts/AuthContext';

type Phase = 'intro' | 'instructions' | 'trial' | 'block-break' | 'results';

const STORAGE_KEY = 'mesearch-weapons-iat-progress';
const INTER_TRIAL_INTERVAL = 250; // ms between trials

export default function WeaponsIATAssessment() {
  const { user } = useAuth();
  const [baseConfig] = useState<IATConfig>(weaponsIAT);
  ...
  // Use base blocks directly (counterbalancing handled by block structure)
  const [blocks] = useState(() => baseConfig.blocks);

  // For backwards compatibility with scoring function
  const counterbalanced = pairingOrderSwapped;

  // Create effective config with blocks
            ...
            <div className="card-premium rounded-lg p-12 max-w-2xl w-full text-center">
              ...
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-4 text-left">
                ...
                <p className="text-[var(--color-text-muted)] text-xs">
                  This assessment examines implicit associations related to race and weapons.
                  The results may be surprising or uncomfortable. Please read the disclaimer carefully before proceeding.
                </p>
              </div>
