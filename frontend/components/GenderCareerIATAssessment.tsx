...
import GenderCareerIATResults from './GenderCareerIATResults';
import { useAuth } from '../contexts/AuthContext';

type Phase = 'intro' | 'instructions' | 'trial' | 'block-break' | 'results';

const STORAGE_KEY = 'mesearch-gender-career-iat-progress';
const INTER_TRIAL_INTERVAL = 250; // ms between trials

export default function GenderCareerIATAssessment() {
  const { user } = useAuth();
  const [baseConfig] = useState<IATConfig>(genderCareerIAT);
  ...
  // 2. targetSideSwapped: Which target starts on the left (Male vs Female)
  ...
  // Note: The counterbalancing function is designed for Flowers/Insects but we use
  // it without that feature for now - just use the base blocks directly
            ...
            <div className="card-premium rounded-lg p-12 max-w-2xl w-full text-center">
              ...
              <div className="space-y-4">
                <p className="text-[var(--color-text-secondary)] text-sm">
                  You will categorize names and words using the E and I keys as quickly as possible.
                </p>
                <button
                  onClick={startIAT}
